...
### Hypothesis:
We hypothesize that the relative risk of dementia of patients with cardiovascular-related conditions compared to healthy individuals (quotient of risk of cardio-vascular patients and risk of healthy individuals) is higher in the age group 65-69 than in other age groups with similar conditions.
...
```
Next, we explored how the dementia risk distribution varies across the two groups with and without prior cardiovascular
conditions. The plot shows that there is a tendency for the peaks at higher dementia risk to belong to the group with cardiovascular conditions.
...
are also older and that age is a significant factor in the risk of dementia. We explore this relationship further next.
```{r}
truncated_data <- cleaned_data %>%
  select(Age_Group, Cardio_Condition, Dementia_risk) %>%
  mutate(Cardio_Condition = ifelse(Cardio_Condition == 1, "With Heart", "Without Heart"),
         # Convert the age group to the average age of that group slighly abusing the fact that the age groups are
         # sorted alphabetically which means that the factors are also sorted that than can be multiplied.
         Age_Group_number = as.numeric(Age_Group)*5 - 3)

ggplot(truncated_data, aes(x = Dementia_risk, fill = Cardio_Condition)) +
  ...
  labs(title = "Predicted Risk of Dementia by Age Group",
       ...
       fill = "Age Group")
```
The following plot showcases the age distributions grouped by the presence of cardiovascular conditions and showcases
...
```{r}
ggplot(truncated_data, aes(x = Age_Group_number, fill = Cardio_Condition)) +
        geom_density(alpha = 0.5) +
        labs(title = "Predicted Risk of Dementia by Age Group",
                x = "Predicted Risk of Dementia",
                y = "Density",
