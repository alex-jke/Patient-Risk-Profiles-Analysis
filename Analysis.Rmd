...
![Causal Relationships](data/Blank diagram - Kausalitätsgraph.svg)
...
data <- data %>%
        mutate(Cardio_Condition = ifelse(
                ...
                Age_Group = ifelse(
                        c60_64 == 1, "60-64",
                        ifelse(c65_69 == 1, "65-69",
                               ifelse(c70_74 == 1, "70-74",
                                      ifelse(c75_79 == 1, "75-79", NA) )
                        )
        )) %>%
        filter(!is.na(Age_Group))
head(data %>% select(personId, Age_Group, Angina, AF, CAD, Cardio_Condition))
```
The data now contains the columns `r colnames(data)` and `r nrow(data)` rows.
## Exploratory Data Analysis
```{r}
ggplot(data, aes(x = Dementia_risk, fill = Age_Group)) +
...
ggplot(data, aes(x = Dementia_risk, y = 1, color = Age_Group, fill = Age_Group)) +
